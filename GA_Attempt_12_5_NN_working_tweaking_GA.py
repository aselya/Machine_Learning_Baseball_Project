...
class Team_chromosomes:
    ...
    def make_new_stat_value(self):
        new_value = 1 + random.randint(-50000,50000)/1000000
...
def plot_top_scores(TOP_SCORES):
    print("plotting top scores")
    #plt.plot(TOP_SCORES)
    #plt.show()
    plot(TOP_SCORES)
    print('continnue compilation')
    show(block=False)
...
#makes the first 4 the parents
    ...
    for score in TOP_SCORES:
        print('Top Score '+str(print_count) +': ' + str(score) )
        score_count =+ 1

    plot_top_scores(TOP_SCORES)
    '''
    print("plot")
    plt.plot(TOP_SCORES)
    plt.show()
